import { ArchimateProject, ArchiDiagram, ArchiDiagramChild, ElementPos, ArchiDiagramObject, ArchiObject, ArchiSourceConnection } from './greeter';

export class DiagramRenderer {
  public static BuildSvg(project: ArchimateProject, diagram: ArchiDiagram, modelTemplate: DiagramTemplate): Document {
    const document = modelTemplate.GetEmptySvg();
    ...
    function AddElement(child: ArchiDiagramChild, parent: Element) {
      ...
      let es = modelTemplate.GetElementByType(archiElement.EntityType)?.outerHTML;
      if (es == null && (archiElement.EntityType.startsWith('Technology') || archiElement.EntityType.startsWith('Application'))) {
        es = modelTemplate.GetElementByType(archiElement.EntityType.replace(/Technology|Application/, 'Business'))?.outerHTML;
      ...
      if (!es)
        es = modelTemplate.GetElementByType('todo').outerHTML;
...
export class DiagramTemplate {
  ...
  private elementSelection: SVGGElement;
